...
const Opportunities = ({ onRouteChange, opportunitiesList = initialOpportunities, scheduledCalls = [] }) => {
  ...
  // Active video call state (student joining a call)
  ...
  // Get scheduled calls that match an opportunity's title
  const getCallsForOpportunity = (op) => {
    return scheduledCalls.filter(
      (call) => call.jobTitle?.toLowerCase() === op.title?.toLowerCase()
    );
  };
  ...
  // Join a call as student
  const handleJoinCall = (call) => {
    setActiveCallRoom(call.roomID);
  };

  // If student is in a video call, show full-screen call
  if (activeCallRoom) {
    return (
      <VideoCall
  ...
  if (selectedOpportunity) {
    ...
    const opCalls = getCallsForOpportunity(selectedOpportunity);
          ...
          <main className="max-w-6xl mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
              ...
              <div className="lg:col-span-2 flex flex-col gap-6">
                ...
                {/* ── Scheduled Video Calls Banner (Detail View) ── */}
                {opCalls.length > 0 && (
                  <div className="bg-gradient-to-r from-violet-50 to-indigo-50 rounded-3xl p-6 sm:p-8 border border-violet-200 shadow-[0_10px_30px_rgba(0,0,0,0.04)]">
                    <div className="flex items-center gap-2 mb-4">
                      <div className="w-8 h-8 rounded-lg bg-violet-600 text-white flex items-center justify-center">
                        <Video size={16} />
                      </div>
                      <h2 className="text-lg font-bold text-slate-900">Scheduled Video Calls</h2>
                    </div>
                    <div className="flex flex-col gap-3">
                      {opCalls.map((call) => (
                        <div
                          key={call.id}
                          className="flex items-center justify-between bg-white rounded-2xl px-5 py-4 border border-violet-100 shadow-sm"
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-bold text-slate-800">
                                Interview with {call.candidateName}
                              </p>
                          ...
                          <button
                            ...
                            className="px-4 py-2 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl transition-all cursor-pointer flex items-center gap-2 shadow-md active:scale-95"
                          >
                            <Video size={15} />
                            Join Call
                          </button>
              ...
              <div className="flex flex-col gap-6 lg:sticky lg:top-24">
                ...
                {/* Join Call Sidebar Widget (if calls exist) */}
                {opCalls.length > 0 && (
                  <div className="bg-gradient-to-br from-violet-600 to-indigo-700 rounded-3xl p-6 text-white shadow-lg">
                    ...
                    <p className="text-violet-200 text-xs leading-relaxed mb-4">
                      You have {opCalls.length} scheduled video call{opCalls.length > 1 ? 's' : ''} for this opportunity. Join when it's time!
                    </p>
                    <button
                      onClick={() => handleJoinCall(opCalls[0])}
                      className="w-full py-2.5 bg-white text-violet-700 font-bold rounded-xl text-sm hover:bg-violet-50 transition-all cursor-pointer active:scale-95 shadow-md"
                    >
                      Join Now
                    </button>
                  </div>
            ...
            {filteredOpportunities.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredOpportunities.map((op) => {
                  ...
                  const opCalls = getCallsForOpportunity(op);
                  const hasCall = opCalls.length > 0;

                  return (
                    <div
                      key={op.id}
                      className={`relative w-full bg-white rounded-2xl p-6 border shadow-[0_10px_30px_rgba(0,0,0,0.06)] hover:shadow-[0_20px_40px_rgba(0,0,0,0.12)] hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between gap-5 ${
                        hasCall ? 'border-violet-200 ring-1 ring-violet-100' : 'border-slate-100'
                      }`}
                    >
                      {/* Video Call Scheduled Badge */}
                      {hasCall && (
                        <div className="absolute -top-2.5 right-4 z-10">
                          <div className="flex items-center gap-1.5 px-3 py-1 bg-violet-600 text-white text-[11px] font-bold rounded-full shadow-md">
                      ...
                      {/* Video Call Join Section (inside card) */}
                      {hasCall && (
                        <div className="bg-violet-50 rounded-xl p-3 border border-violet-100">
                          {opCalls.map((call) => (
                            <div key={call.id} className="flex items-center justify-between">
                              ...
                              <button
                                onClick={() => handleJoinCall(call)}
                                className="px-3 py-1.5 text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg transition-all cursor-pointer flex items-center gap-1.5 shadow-sm active:scale-95"
                              >
                                <Video size={12} />
                                Join
                              </button>
